...
import net.dasm.provider.ClassProvider;
import net.dasm.redirect.AddFieldToSets;
import net.dasm.redirect.AddMethodToSets;
import net.dasm.redirect.DasmRedirectSet;
import net.dasm.redirect.PartialRedirect;
import net.dasm.transform.DasmRedirect;
import net.dasm.transform.TransformFrom;
import net.dasm.transform.TransformFromClass;
import net.dasm.transformer.ClassField;
import net.dasm.transformer.ClassMethod;
import net.dasm.transformer.redirect.FieldRedirect;
import net.dasm.transformer.redirect.MethodRedirect;
import net.dasm.transformer.redirect.RedirectSet;
import net.dasm.transformer.redirect.TypeRedirect;
import net.dasm.transformer.target.TargetClass;
import net.dasm.transformer.target.TargetMethod;
import net.dasm.util.Pair;
...
public class AnnotationParser {
    ...
    private List<RedirectSet> getRedirectSetsForType(Type setType) {
        return this.redirectSetsByType.computeIfAbsent(setType, t -> {
            ...
            for (InnerClassNode innerClass : classNode.innerClasses) {
                ...
                if (isAnnotationIfPresent(innerClassNode.invisibleAnnotations, net.dasm.redirect.TypeRedirect.class)) {
    ...
    private MethodRedirect parseMethodRedirect(MethodNode methodNode, Type owner, @Nullable Type newOwner, boolean isDstInterface) {
        for (AnnotationNode annotation : methodNode.invisibleAnnotations) {
            if (!annotation.desc.equals(classToDescriptor(net.dasm.redirect.MethodRedirect.class))) {
                continue;
            }

            Map<String, Object> values = getAnnotationValues(annotation, net.dasm.redirect.MethodRedirect.class);
    ...
    private FieldRedirect parseFieldRedirect(FieldNode fieldNode, Type owner, @Nullable Type newOwner) {
        for (AnnotationNode annotation : fieldNode.invisibleAnnotations) {
            if (!annotation.desc.equals(classToDescriptor(net.dasm.redirect.FieldRedirect.class))) {
                continue;
            }
    ...
    private TypeRedirect parseTypeRedirect(ClassNode innerClass) {
        for (AnnotationNode annotation : innerClass.invisibleAnnotations) {
            if (!annotation.desc.equals(classToDescriptor(net.dasm.redirect.TypeRedirect.class))) {
                continue;
            }
